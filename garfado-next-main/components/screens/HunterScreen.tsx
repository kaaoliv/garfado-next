  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-hide">
      <div className="px-4 pt-2 pb-6">
        ...
        {/* Descrição */}
        <p className="text-sm text-muted-foreground mb-4">Quantas unidades no Brasil você já foi?</p>
        ...
        <div className="bg-card rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-primary font-bold text-sm">
              {totalGarfadas}/{totalBrasil.toLocaleString('pt-BR')}
            </span>
          </div>
          <div className="w-full h-3 bg-secondary rounded-full overflow-hidden">
            <div className="h-full bg-primary rounded-full transition-all"
              style={{ width: `${Math.round(totalGarfadas / totalBrasil * 100)}%` }} />
          </div>
          <p className="text-xs text-muted-foreground mt-2">
            `${Math.round(totalGarfadas / totalBrasil * 100)}% de todas as unidades no Brasil`
          </p>
        </div>
                ...
                <button onClick={() => setOpenRede(isOpen ? null : rede)}
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {garfadas.length} de {totalRef.toLocaleString('pt-BR')} unidades no Brasil 🇧🇷
                      </p>
                    </div>
                  ...
                  {!isComplete && (
                    <p className="text-[11px] text-muted-foreground mt-2">
                      Faltam {(totalRef - garfadas.length).toLocaleString('pt-BR')} unidades para completar
                    </p>
                  )}
                </button>
