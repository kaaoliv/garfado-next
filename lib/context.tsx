...
interface AppState {
  ...
  ratings: Record<number, { Comida?: number; Atendimento?: number; Limpeza?: number; Banheiro?: number; nota?: string; price_level?: number; delivery?: boolean; Embalagem?: number; Pontualidade?: number }>
...
interface AppActions {
  ...
  setDelivery: (rid: number, delivery: boolean) => Promise<void>
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  const [state, setState] = useState<AppState>({
    ...
    placesCache: {}, searchResults: [], trending: [], placesSessionToken: crypto.randomUUID(), loading: true, onboarding: false,
  })
  ...
  const loadDataFor = async (uid: string) => {
    ...
    const userPhotoMap: Record<number, string> = {}
    ;(vR.data || []).forEach((v: any) => {
      visits[v.restaurant_id] = v.count || 1
      if (v.updated_at) visitDates[v.restaurant_id] = v.updated_at
      if (v.photo_url) userPhotoMap[v.restaurant_id] = v.photo_url
    })
    const ratings: Record<number, any> = {}
    ;(rR.data || []).forEach((r: any) => { ratings[r.restaurant_id] = { Comida: r.comida, Atendimento: r.atendimento, Limpeza: r.limpeza, Banheiro: r.banheiro, nota: r.nota, price_level: r.price_level, delivery: r.delivery, Embalagem: r.embalagem, Pontualidade: r.pontualidade } })
  ...
  const addVisit = async (rid: number, delta: number) => {
    ...
    if (next === 0) {
      ...
      // Notificação: primeira vez nesse lugar → checar se amigos também foram
      if (prev === 0) {
        const friendsWhoWent = state.friendVisits[rid] || []
        if (friendsWhoWent.length > 0) {
          const { toast } = await import('sonner')
          const names = friendsWhoWent.slice(0, 2).map(f => f.name.split(' ')[0])
          const others = friendsWhoWent.length > 2 ? ` e mais ${friendsWhoWent.length - 2}` : ''
          const rName = r?.name || 'aqui'
          toast(`🍴 ${names.join(', ')}${others} também ${friendsWhoWent.length === 1 ? 'foi' : 'foram'} em ${rName}!`, {
            duration: 5000,
            position: 'top-center',
          })
        }
  ...
  const setRating = async (rid: number, field: string, val: number) => {
    ...
    await supabase.from('ratings').upsert({
      ...
      price_level: field === 'price_level' ? val : mr.price_level || null,
      embalagem: field === 'Embalagem' ? val : mr.Embalagem || null,
      pontualidade: field === 'Pontualidade' ? val : mr.Pontualidade || null,
      nota: mr.nota || null, updated_at: new Date().toISOString(),
    }, { onConflict: 'user_id,restaurant_id' })
  }

  const setDelivery = async (rid: number, delivery: boolean) => {
    setState(s => ({ ...s, ratings: { ...s.ratings, [rid]: { ...s.ratings[rid], delivery } } }))
    const mr = state.ratings[rid] || {}
    await supabase.from('ratings').upsert({
      user_id: state.user!.id, restaurant_id: rid,
      delivery,
      ...
      price_level: mr.price_level || null, embalagem: mr.Embalagem || null,
      pontualidade: mr.Pontualidade || null, nota: mr.nota || null,
  ...
  const searchPlaces = async (q: string, coords?: { lat: number; lng: number }) => {
    ...
    try {
      ...
      if (cached?.results) {
        update({ searchResults: cached.results as PlaceResult[] })
        return
      }
  ...
  const addFromPlaces = async (place: PlaceResult): Promise<number | null> => {
    ...
    // Busca de foto DESABILITADA — campo 'photos' é cobrado pela Places API
    // Usuário pode adicionar foto manualmente pelo botão de câmera no modal
    let photoUrl: string | null = null

    // 1. Verificar se já existe pelo place_id
    if (place.placeId) {
      const { data: ex } = await supabase.from('restaurants').select('id,img').eq('place_id', place.placeId).maybeSingle()
      if (ex) {
        rid = ex.id
        // Atualizar foto se não tinha
        if (!ex.img && photoUrl) {
          await supabase.from('restaurants').update({ img: photoUrl }).eq('id', rid)
        }
        setState(s => {
          if (s.restaurants.find(r => r.id === rid)) return s
          const newR = { id: rid!, name: place.name, addr: place.addr, rede, rating: place.rating, img: photoUrl || ex.img, hours: '–', place_id: place.placeId || null }
    ...
    if (!rid) {
      const { data: ins, error } = await supabase.from('restaurants').insert({
        ...
        img: photoUrl, place_id: place.placeId || null, hours: '–',
      }).select().single()

      if (ins && !error) {
        rid = ins.id
        setState(s => ({ ...s, restaurants: [...s.restaurants, { ...ins, img: photoUrl || ins.img }] }))
      } else {
        // 3. Fallback: usar ID temporário em memória se Supabase falhar
        const tempId = Date.now()
        const tempR = { id: tempId, name: place.name, addr: place.addr, rede, rating: place.rating, img: photoUrl, hours: '–', place_id: place.placeId || null }
        setState(s => ({ ...s, restaurants: [...s.restaurants, tempR] }))
        rid = tempId
      }
    }

    // Renovar session token após seleção (nova sessão de busca)
    update({ searchResults: [], placesSessionToken: crypto.randomUUID() })
    return rid
  }

  // fetchPlacePhoto DESABILITADO — evitar cobranças da API
  const fetchPlacePhoto = async (_r: Restaurant) => { return }
  ...
  const uploadRestaurantPhoto = async (rid: number, file: File): Promise<boolean> => {
    ...
    try {
      ...
      const path = isAdmin
        ? `restaurants/${rid}.jpg`
        : `users/${userId}/restaurants/${rid}.jpg`

      // Detectar tipo correto (iOS Safari às vezes envia type vazio)
      const mime = file.type && file.type.startsWith('image/') ? file.type : 'image/jpeg'

      // Deletar anterior e fazer upload
      await supabase.storage.from('restaurant-photos').remove([path])
      const { error, data: uploadData } = await supabase.storage.from('restaurant-photos').upload(
        path, file, { upsert: true, contentType: mime, duplex: 'half' }
      )
      if (error) {
      ...
      // Salvar URL — admin salva no restaurante global, outros na visita
      if (isAdmin) {
        const { error: e2 } = await supabase.from('restaurants').update({ img: url }).eq('id', rid)
        if (e2) throw e2
      } else {
        // Salvar na coluna photo_url da visita E no estado local
        await supabase.from('visits').update({ photo_url: url })
          .eq('user_id', userId).eq('restaurant_id', rid)
      }
      ...
      console.error('uploadRestaurantPhoto:', e)
      const { toast } = await import('sonner')
  ...
  const updateFriends = (friends: string[]) => update({ friends })
  ...
  return (
    <AppContext.Provider value={{
      ...
      addVisit, toggleLike, setRating, setNote, setDelivery,
      searchPlaces, clearSearch, addFromPlaces, fetchPlacePhoto,
      reactFeed, loadReviews, submitReview, deleteReview,
      finishOnboarding, signOut, setProfile, uploadRestaurantPhoto, updateFriends,
    }}>
      {children}
    </AppContext.Provider>
  )
