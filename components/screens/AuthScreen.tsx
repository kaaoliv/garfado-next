...
export function AuthScreen() {
  ...
  const features = [
    { ico: '◎', key: 'auth.feature1' },
    { ico: '⭐', key: 'auth.feature2' },
    { ico: '👥', key: 'auth.feature3' },
    { ico: '🎯', key: 'auth.feature4' },
  ]
    ...
    <main className="flex-1 flex flex-col items-center justify-center px-8 gap-8">
      ...
      >
        {features.map((f, i) => (
          ...
          >
            <span className="text-lg w-7 text-center flex-shrink-0">{f.ico}</span>
            <span>{t(f.key)}</span>
          </motion.li>
        ))}
      </motion.ul>
      ...
      >
        ...
        >
          ...
          <span>{loading ? t('auth.connecting') : t('auth.google')}</span>
        </motion.button>
        <p className="text-center text-xs text-muted-foreground">
          {t('auth.terms_before')}{' '}
          <a href="https://garfado.com.br/politica-privacidade"
