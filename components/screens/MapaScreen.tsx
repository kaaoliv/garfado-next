...
export function MapaScreen({ onOpenModal }: MapaScreenProps) {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      // Tile dinâmico — dark/light baseado no tema
      const getMapTile = () => {
        const theme = localStorage.getItem('garfado-theme') || 'dark'
        const isDark = theme !== 'light'
        return isDark
          ? 'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png'
          : 'https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png'
      }
      ;(window as any).__getMapTile = getMapTile
      const tileLayerRef = L.tileLayer(getMapTile(), {
        maxZoom: 19,
        subdomains: 'abcd',
        crossOrigin: true,
      }).addTo(map)
      ;(window as any).__garfadoTileLayer = tileLayerRef
      ;(window as any).__getMapTile = getMapTile
    ...
    <div className="flex-1 relative overflow-hidden">
      ...
      {loading && (
        <div className="absolute inset-0 z-20 bg-background flex flex-col items-center justify-center gap-4 px-8 text-center">
          ...
          <div>
            <p className="text-sm font-medium">{t('map.locating')}</p>
      ...
      {!loading && (
        <div className="absolute top-14 left-0 right-0 z-10 flex gap-2 px-4 pointer-events-none">
          {[
            { id: 'todos', label: t('map.all') },
            { id: 'visitados', label: t('map.visited') },
            { id: 'nao-visitados', label: t('map.not_visited') },
          ].map(f => (
      ...
      {!loading && (
        <div className="absolute top-14 right-4 z-10 bg-black/70 backdrop-blur-md rounded-xl p-2.5 border border-white/10 text-xs shadow-lg">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-white/80">{t('map.visited_label')}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full bg-[#6b7280] flex-shrink-0" />
            <span className="text-white/50">{t('map.not_visited_label')}</span>
          </div>
        </div>
      )}
      ...
      {!loading && pins.length === 0 && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 pointer-events-none">
          <div className="bg-black/70 backdrop-blur-md rounded-2xl px-6 py-4 text-center border border-white/10">
            ...
            <p className="text-sm text-white/60">{t('map.no_address')}</p>
          </div>
        </div>
      )}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2">
                {selected.visited
                  ...
                  : <span className="text-xs bg-secondary text-muted-foreground px-2 py-0.5 rounded-full">{t('map.not_visited_label')}</span>
                }
              </div>

              {!selected.visited && friendsWhoWent.length > 0 && (
                <div className="mt-2.5 border-t border-border pt-2">
                  <p className="text-[10px] text-muted-foreground mb-1.5 uppercase tracking-wide font-medium">{t('map.friends_went')}</p>
