...
export function AmigosScreen({ onOpenModal, onViewProfile }: AmigosScreenProps) {
  ...
  const toggleFollow = async (uid: string) => {
    if (pendingFollows.has(uid)) return
    const isFollowing = friends.includes(uid)
    // Otimista — atualiza na hora
    setPendingFollows(prev => new Set(prev).add(uid))
    setState(s => ({
      ...s,
      friends: isFollowing ? s.friends.filter(id => id !== uid) : [...s.friends, uid]
    }))
    try {
      if (isFollowing) {
        ...
        toast.success(t('friends.following_toast'))
      }
    } catch {
      // Reverter se falhar
      updateFriends(isFollowing ? [...friends, uid] : friends.filter(id => id !== uid))
    }
    setPendingFollows(prev => { const n = new Set(prev); n.delete(uid); return n })
  }

  const tabs: { id: SubTab; label: string }[] = [
    { id: 'feed', label: t('friends.feed') },
    { id: 'ranking', label: t('friends.ranking') },
    { id: 'pessoas', label: t('friends.people') },
  ]
              ...
              return (
                <div key={u.id} className="flex items-center gap-3 p-2 rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{u.name || t('friends.no_name')}</p>
                  ...
                  <button onClick={() => toggleFollow(u.id)}
                    ...
                    {isF ? t('friends.following') : t('friends.follow')}
                  </button>
                </div>
              )
            })}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
          ...
          <div className="p-4 flex flex-col gap-3">
            {feed.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
                ...
                <p className="font-serif text-base font-semibold">{t('friends.feed_empty')}</p>
                <p className="text-sm text-muted-foreground">{t('friends.feed_empty_sub')}</p>
              </div>
            ) : feed.map(v => {
              ...
              return (
                <div key={`${v.user_id}-${v.restaurant_id}`} className="bg-card rounded-xl p-4">
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <button onClick={() => onViewProfile(pr.id)} className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-sm font-bold text-primary-foreground overflow-hidden outline-2 outline-primary/30 outline flex-shrink-0 touch-manipulation">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t('friends.forked')} <span className="text-foreground font-medium">{v.restaurant_name}</span>
        ...
        {tab === 'ranking' && (
          <div className="p-4">
            {ranking.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
                <span className="text-4xl">🏅</span>
                <p className="font-serif text-base font-semibold">{t('friends.ranking_empty')}</p>
                <p className="text-sm text-muted-foreground">{t('friends.ranking_empty_sub')}</p>
              </div>
            ) : (
                ...
                {ranking.map((entry, i) => {
                  ...
                  return (
                    <div key={p.id}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium ${entry.isMe ? 'text-primary' : ''}`}>{entry.isMe ? t('friends.you') : p.name || '?'}</p>
                        <p className="text-xs text-muted-foreground">@{p.username || '?'}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className={`text-lg font-bold ${entry.isMe ? 'text-primary' : ''}`}>{entry.garfCount}</p>
                        <p className="text-[10px] text-muted-foreground">{t('perfil.garfados')}</p>
                      </div>
                    </div>
                  )
                })}
        ...
        {tab === 'pessoas' && (
          <div className="p-4">
            ...
            {followers.length > 0 && (
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wide mb-2">{t('friends.followers_count').replace('{n}', String(followers.length))}</p>
                <div className="flex flex-col gap-2">
                  {followers.map(u => {
                    const ini = (u.name || '?').charAt(0).toUpperCase()
                    const isF = friends.includes(u.id)
                    return (
                      <div key={u.id} className="flex items-center gap-3 bg-card rounded-xl p-3">
                        <button onClick={() => onViewProfile(u.id)} className="flex items-center gap-3 flex-1 min-w-0 text-left touch-manipulation">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium">{u.name || t('friends.no_name')}</p>
                            <p className="text-xs text-muted-foreground">@{u.username || '?'}</p>
                          </div>
                        </button>
                        <button onClick={() => toggleFollow(u.id)}
                          ...
                          {isF ? t('friends.following') : 'Seguir back'}
                        </button>
                      </div>
                    )
                  })}
            ...
            {followers.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
                ...
                <p className="font-serif text-base font-semibold">{t('friends.no_followers')}</p>
                <p className="text-sm text-muted-foreground">{t('friends.no_followers_sub')}</p>
              </div>
            )}
          </div>
        )}
